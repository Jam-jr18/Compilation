@extends('layout')
@section('title', $business['name'].' - order')
@push('head')
<style>
...
@endphp
<form method="POST" action="{{ route('orders.store') }}" id="orderForm" class="pos-shell">
    @csrf
    <input type="hidden" name="order_type" id="orderType" value="{{ old('order_type', 'dine_in') }}">
    ...
    <section class="menu-pane">
        ...
        <nav class="category-strip">
            ...
            @foreach($categories as $category)
                <a href="#cat-{{ $category->id }}" class="cat-pill">{{ $category->name }}</a>
            @endforeach
        </nav>

        <div class="menu-content" id="all">
            @foreach($allItems as $item)
                <input type="hidden" class="qty-input" data-item-id="{{ $item->id }}" name="quantities[{{ $item->id }}]" value="{{ old('quantities.'.$item->id, 0) }}">
            @endforeach
            ...
            @foreach($categories as $category)
                <section class="category-block" id="cat-{{ $category->id }}">
                    <div class="category-title">
                        ...
                        <span class="option-badge">{{ $category->items->count() }} Options</span>
                    </div>
                    <div class="product-grid">
                        @forelse($category->items as $item)
                            <article class="product-card">
                                <span class="price-bubble">₱{{ number_format($item->price, 0) }}</span>
                                ...
                                <div class="product-body">
                                    ...
                                    <button type="button" class="add-btn" data-add="{{ $item->id }}"><span style="font-size:28px;line-height:0">+</span> Add to Order</button>
                                </div>
                            </article>
                        @empty
    ...
    <aside class="cart-pane">
        ...
        <div class="checkout-box" id="checkoutBox" style="display:none">
            <div class="field">
                ...
                <input class="input" name="customer_name" value="{{ old('customer_name') }}" placeholder="optional">
            </div>
            <div class="field" id="tableField">
                ...
                <select class="select" name="restaurant_table_id">
                    ...
                    @foreach($tables as $table)
                        <option value="{{ $table->id }}" @selected(old('restaurant_table_id') == $table->id)>{{ $table->name }} • {{ $table->capacity }} pax</option>
                    @endforeach
                </select>
            </div>
            <div class="field">
                ...
                <select class="select" name="payment_method" id="paymentMethod">
                    ...
                    <option value="ewallet" @selected(old('payment_method') === 'ewallet')>GCash / E-wallet</option>
                </select>
            </div>
            <div id="ewalletFields" style="display:none">
                @if($business['gcash_qr_base64'])
                    <img class="qr-mini" src="{{ $business['gcash_qr_base64'] }}" alt="GCash QR">
                @endif
                <div class="field">
                    <label>Sender name</label>
                    <input class="input" name="payment_sender_name" value="{{ old('payment_sender_name') }}">
                </div>
                <div class="field">
                    <label>Reference number</label>
                    <input class="input" name="payment_reference" value="{{ old('payment_reference') }}">
                </div>
            </div>
            <div class="field">
                <label>Notes</label>
                <textarea class="textarea" name="notes" placeholder="optional request">{{ old('notes') }}</textarea>
            </div>
...
@push('scripts')
<script>
const menuItems = @json($allItems->mapWithKeys(fn($item) => [$item->id => ['name' => $item->name, 'price' => (float) $item->price]])->toArray());
const quantities = {};
